...
DB_CONN = {
'edu_mit_media_funf_probe_builtin_BluetoothProbe' : 'bluetooth',
...
def print_stats(db_qual, column):
    ...
    data_file_name = "data" + column + "values.tsv"
    column_names_data = "username\tall\tallcount\tmonth\tmonthcount\tweek\tweekncount\n"
    ...
    with open(data_file_name, 'w') as f_handle:
        f_handle.write(column_names_data)
        for user in users_list:
            # get user stats all / month / week for quality specified as column
            cur = db_qual.cursor()
            query = "SELECT all_count, all_max, month_count, week_count FROM stats_daily WHERE user=%s and type=%s"
            ...
            cur.close()
            if len(result) > 0:
                row = user[0] + "\t" + str(result[0][0]) + "\t" + str(result[0][1]) + "\t" + str(result[0][2]) + "\t" + str(MONTH_EXP) + "\t" + str(result[0][3]) + "\t" + str(WEEK_EXP) + "\n"
                f_handle.write(row)
...
for connection in DB_CONN.keys():
    ...
    users_list = get_users_list(db_data, "researcher")
    user_idx = 0
    before = datetime.now()
    ...
    db_data.close()
    print_stats(db_quality, DB_CONN[connection])
